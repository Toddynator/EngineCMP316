...
namespace CMP316engine
{
    VoxelResource VoxImporter::LoadVox(const char* filepath)
	{
        ...
        bool customPaletteExists = false;
		while (file.tellg() < filesize)
		{
            ...
            else if (chunkIDString == "RGBA") // This is how the colour index gets the correct colour (index based on position in pallete).
            {
                ...
                for (int i = 0; i < 256; i++)
                {
                    ...
                    colourPalette[i] = (int)((alpha << 24) | (red << 16) | (green << 8) | blue);
                }
            }
            else // UNKNOWN CHUNK
            {
        ...
        if (!customPaletteExists)
        {
            ...
            std::array<int, 256> defaultColourPalette = {
